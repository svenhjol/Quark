...
public class CompassAngleGetter {
	...
	public static void tickCompass(PlayerEntity player, ItemStack stack) {
		...
		boolean nether = player.world.getRegistryKey().getValue().equals(DimensionOptions.NETHER.getValue());
	...
	public static class Impl implements ModelPredicateProvider {
		...
		public float call(@Nonnull ItemStack stack, @Nullable ClientWorld worldIn, @Nullable LivingEntity entityIn) {
			...
			Identifier dimension = worldIn.getRegistryKey().getValue();
			BlockPos lodestonePos = CompassItem.hasLodestone(stack) ? this.getLodestonePosition(worldIn, stack.getOrCreateTag()) : null;
			
			if(lodestonePos != null) {
				calculate = true;
				target = lodestonePos;
			} else if(dimension.equals(DimensionOptions.END.getValue()) && CompassesWorkEverywhereModule.enableEnd) 
				calculate = true;
			else if(dimension.equals(DimensionOptions.END.getValue()) && isCalculated(stack) && CompassesWorkEverywhereModule.enableNether) {
			...
			} else if(worldIn.getDimension().isNatural()) {
				calculate = true;
		...
		private BlockPos getLodestonePosition(World p_239442_1_, CompoundTag p_239442_2_) {
			...
			if (flag && flag1) {
				...
				if (optional.isPresent() && p_239442_1_.getRegistryKey().equals(optional.get())) {
