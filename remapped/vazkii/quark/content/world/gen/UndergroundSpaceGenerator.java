...
public class UndergroundSpaceGenerator extends ClusterBasedGenerator {
	...
	public IGenerationContext createContext(BlockPos src, ChunkGenerator generator, Random random, BlockPos chunkCorner, ChunkRegion world) {
		return (pos, caveNoise) -> {
			...
			if(state.getHardness(world, pos) > -1 || state.getMaterial() == Material.LAVA) {
				if(pos.getY() < 6){
	 ...
	 */
	private static void removeInvalidBlocksAndEncloseLava(BlockPos src, ChunkRegion world, BlockState state){
		BlockPos.Mutable mutable = new BlockPos.Mutable().set(src);

		for(Direction direction : Direction.values()){
			mutable.set(src).move(direction);
			...
			if(direction == Direction.UP || direction == Direction.DOWN){
				while(mutable.getY() < world.getHeight() && mutable.getY() > 0 && !world.getBlockState(mutable).canPlaceAt(world, mutable)){
				...
				// for sides only
				if(!world.getBlockState(mutable).canPlaceAt(world, mutable)){
					world.setBlockState(mutable, Blocks.CAVE_AIR.getDefaultState(), 2);
				}
			}
	...
	private static boolean noWaterNeighbor(BlockPos src, ChunkRegion world){
		...
		for(Direction direction : Direction.values()){
			...
			if(direction != Direction.DOWN){
				BlockState neighborBlock = world.getBlockState(mutable.set(src).move(direction));
				...
				if(neighborBlock.getMaterial() == Material.WATER && mutable.getY() > world.getTopPosition(Heightmap.Type.OCEAN_FLOOR, mutable).getY() - 2){
					return false;
				}
			}
		}
