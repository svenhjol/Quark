...
public class EnchantedBookTooltips {
	...
	public static void renderTooltip(RenderTooltipEvent.PostText event) {
		...
		MinecraftClient mc = MinecraftClient.getInstance();
		List<? extends StringVisitable> tooltip = event.getLines();
	...
	private static void computeTestItems() {
		...
		for (String loc : ImprovedTooltipsModule.enchantingStacks) {
			Registry.ITEM.getOrEmpty(new Identifier(loc)).ifPresent(item -> testItems.add(new ItemStack(item)));
		}
	}
	...
	private static void computeAdditionalStacks() {
		...
		for(String s : ImprovedTooltipsModule.enchantingAdditionalStacks) {
			...
			Optional<Enchantment> ench = Registry.ENCHANTMENT.getOrEmpty(new Identifier(left));
			if(ench.isPresent()) {
				tokens = right.split(",");

				for(String itemId : tokens) {
					Registry.ITEM.getOrEmpty(new Identifier(itemId)).ifPresent(item -> additionalStacks.put(ench.get(), new ItemStack(item)));
				}
			}
		}
	}
