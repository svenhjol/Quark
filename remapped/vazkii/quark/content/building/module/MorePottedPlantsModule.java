...
public class MorePottedPlantsModule extends QuarkModule {
	...
	public void construct() {
		...
		VariantHandler.addFlowerPot(Blocks.SEA_PICKLE, "sea_pickle", p -> p.luminance(b -> 3));
	...
	public void clientSetup() {
		for(Block b : tintedBlocks.keySet()) {
			...
			BlockColorProvider color = (state, worldIn, pos, tintIndex) -> MinecraftClient.getInstance().getBlockColors().getColor(tState, worldIn, pos, tintIndex);
			MinecraftClient.getInstance().getBlockColors().registerColorProvider(color, b);
		}
	}
