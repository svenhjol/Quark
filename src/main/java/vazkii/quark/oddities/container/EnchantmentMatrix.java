...
public class EnchantmentMatrix {
	...
	private EnchantmentDataWrapper generateRandomEnchantment(Map<Enchantment, Integer> influences, int bookshelfPower) {
		...
		for(Enchantment enchantment : ForgeRegistries.ENCHANTMENTS.getValues()) {
			if ((!enchantment.isTreasure() || MatrixEnchantingModule.allowTreasures)
		...
		return WeightedPicker.getRandom(rng, validEnchants);
	}
	...
	public void readFromNBT(CompoundTag cmp) {
		pieces.clear();
		totalValue.clear();
		ListTag plist = cmp.getList(TAG_PIECES, cmp.getType());
	...
	public static class Piece {
		...
		public void readFromNBT(CompoundTag cmp) {
			...
			enchant = ForgeRegistries.ENCHANTMENTS.getValue(new Identifier(cmp.getString(TAG_ENCHANTMENT)));
	...
	private static class EnchantmentDataWrapper extends EnchantmentLevelEntry {
			...
			if(MatrixEnchantingModule.normalizeRarity) {
				...
				for(Piece other : markedEnchants) {
					if(other.enchant == enchantment) {
						...
						mark = false;
						break;
					} else if(!other.enchant.canCombine(enchantment) || !enchantment.canCombine(other.enchant)) {
