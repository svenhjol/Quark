...
public class MagnetizedBlockTileEntity extends BlockEntity implements Tickable {
    ...
    private void moveCollidedEntities(float progress) {
        ...
        if (!collision.isEmpty()) {
            ...
            List<Entity> entities = this.world.getEntities(null, this.getMovementArea(containingBox, direction, movement).union(containingBox));
            if (!entities.isEmpty()) {
    ...
    public BlockEntity getSubTile() {
        if (subTile != null && !subTile.isEmpty()) {
            ...
            return BlockEntity.createFromTag(magnetState, subTile); // create
        }
        ...
        if (this.lastProgress >= 1.0F) {
            ...
            if (this.magnetState != null && this.world.getBlockState(this.pos).getBlock() == MagnetsModule.magnetized_block) {
                ...
                if (blockstate.isAir()) {
                    ...
                    Block.replaced(this.magnetState, blockstate, this.world, this.pos, 3);
                } else {
