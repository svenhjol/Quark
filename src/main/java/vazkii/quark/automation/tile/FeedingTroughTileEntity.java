...
public class FeedingTroughTileEntity extends LootableContainerBlockEntity implements Tickable {
        ...
        if (foodHolder != null) {
            for (int i = 0; i < size(); i++) {
                ...
                if (goal.isTemptedBy(stack) && entity.isBreedingItem(stack)) {
                    ...
                    Vec3d position = new Vec3d(pos.getX(), pos.getY(), pos.getZ()).add(0.5, 0.5, 0.5);
        ...
        if (world != null && !world.isClient) {
            if (cooldown > 0)
                ...
                for (AnimalEntity creature : animals) {
                    if (creature.isReadyToBreed() && creature.getBreedingAge() == 0) {
                        for (int i = 0; i < size(); i++) {
                            ItemStack stack = getStack(i);
    ...
    @Override
    public void fromTag(BlockState state, CompoundTag nbt) { // read
