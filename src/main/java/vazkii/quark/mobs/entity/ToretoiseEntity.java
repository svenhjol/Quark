...
public class ToretoiseEntity extends AnimalEntity {
	...
	@Override
	public EntityData initialize(WorldAccess p_213386_1_, LocalDifficulty p_213386_2_, SpawnReason p_213386_3_, EntityData p_213386_4_, CompoundTag p_213386_5_) {
		popOre(true);
		return p_213386_4_;
	}
		...
		if(angeryTicks > 0 && isAlive()) {
			...
			if(onGround) {
				...
				if(angeryTicks == 0) {
					...
					List<LivingEntity> hurtMeDaddy = world.getEntities(LivingEntity.class, hurtAabb, e -> !(e instanceof ToretoiseEntity));
	...
	public static boolean spawnPredicate(EntityType<? extends ToretoiseEntity> type, WorldAccess world, SpawnReason reason, BlockPos pos, Random rand) {
    ...
    public static DefaultAttributeContainer.Builder prepareAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, 60.0D) // MAX_HEALTH
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.08D) // MOEVMENT_SPEED
                .add(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, 1.0D); // KNOCKBACK_RESISTANCE
    }
