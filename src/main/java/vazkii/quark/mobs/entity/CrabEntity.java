...
public class CrabEntity extends AnimalEntity implements IEntityAdditionalSpawnData {
	...
	public static DefaultAttributeContainer.Builder prepareAttributes() {
		return MobEntity.createMobAttributes()
				.add(EntityAttributes.GENERIC_MAX_HEALTH, 20.0D) // MAX_HEALTH
				.add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.25D) // MOEVMENT_SPEED
				.add(EntityAttributes.GENERIC_ARMOR, 3.0D) // ARMOR
				.add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, 2.0D) // ARMOR_TOUGHNESS
				.add(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, 0.5D); // KNOCKBACK_RESISTANCE
	}
	...
	public void onStruckByLightning(LightningEntity lightningBolt) {
		...
		if (sizeMod <= 15) {
			...
			this.getAttributeInstance(EntityAttributes.GENERIC_MAX_HEALTH).addTemporaryModifier(new EntityAttributeModifier("Lightning Bonus", 0.5, Operation.ADDITION)); // MAX_HEALTH
			this.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).addTemporaryModifier(new EntityAttributeModifier("Lightning Debuff", -0.05, Operation.ADDITION)); // MOVEMENT_SPEED
			this.getAttributeInstance(EntityAttributes.GENERIC_ARMOR).addTemporaryModifier(new EntityAttributeModifier("Lightning Bonus", 0.125, Operation.ADDITION)); // ARMOR
	...
	@Nullable
	@Override
	public PassiveEntity createChild(@Nonnull PassiveEntity other) {
